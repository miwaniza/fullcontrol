...
def relative_point(reference: Union[Point, list], x_offset: float, y_offset: float, z_offset: float):
    ...
    if pt == None:
        raise Exception(
            f'The reference object must be a Point or a list containing at least one point')
    if None in [pt.x, pt.y, pt.z]:
        raise Exception(f'The reference point must have all of x, y, z attributes defined (x={
                        pt.x}, y={pt.y}, z={pt.z})')
    new_pt = deepcopy(pt)
...
def export_design(steps: list, filename: str):
    ...
    with open(filename + '.json', 'w', encoding='utf-8') as f:
        json.dump(steps, f, ensure_ascii=False, indent=4, default=lambda x: {
                  'type': type(x).__name__, 'data': x.__dict__})
