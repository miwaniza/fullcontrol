...
def rectangleXY(start_point: Point, x_size: float, y_size: float, cw: bool = False) -> list:
    ...
    """
    point1 = Point(x=start_point.x+x_size*(not cw), y=start_point.y +
                   y_size*cw, z=start_point.z)  # boolean False=0
    point2 = Point(x=start_point.x+x_size,
                   y=start_point.y+y_size, z=start_point.z)
    point3 = Point(x=start_point.x+x_size*cw, y=start_point.y +
                   y_size*(not cw), z=start_point.z)
...
def circleXY_3pt(pt1: Point, pt2: Point, pt3: Point, start_angle: float, segments: int = 100, cw: bool = False) -> list:
    ...
    '''
    D = 2 * (pt1.x * (pt2.y - pt3.y) + pt2.x *
             (pt3.y - pt1.y) + pt3.x * (pt1.y - pt2.y))
    if D == 0:
        raise Exception('The points are collinear, no unique circle')
    x_centre = ((pt1.x**2 + pt1.y**2) * (pt2.y - pt3.y) + (pt2.x**2 + pt2.y**2)
                * (pt3.y - pt1.y) + (pt3.x**2 + pt3.y**2) * (pt1.y - pt2.y)) / D
    y_centre = ((pt1.x**2 + pt1.y**2) * (pt3.x - pt2.x) + (pt2.x**2 + pt2.y**2)
                * (pt1.x - pt3.x) + (pt3.x**2 + pt3.y**2) * (pt2.x - pt1.x)) / D
